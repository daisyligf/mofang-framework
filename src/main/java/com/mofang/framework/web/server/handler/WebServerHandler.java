...
public class WebServerHandler extends ChannelInboundHandlerAdapter
{
	...
	public WebServerHandler(PostDataParserType parserType)
	{
    ...
    {
	    	// 暂时先不处理inactive的channel，通过read_idle来进行剔除
	    	/*
	    	Channel channel=ctx.channel();
	    	boolean isOpen = channel.isOpen();
	    	boolean isActive = channel.isActive();
	    	boolean isWritable = channel.isWritable();
	    	System.out.println("time: " + format.format(new Date()) + " channel inactive. isOpen:" + isOpen + " isActive:" + isActive + " isWritable:" + isWritable);
	    	//websocketClose(ctx);
	        //channel.close();
         * */
